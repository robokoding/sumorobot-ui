snack.wrap.define('draggableList', function(config){
  ...
  var movePlaceholder = function(event){
    ...
    if(intersect(event.pageX, event.pageY, target)){
      // enemy and line can be only used in 'if' and functions in 'func'
      var type = dragEl.getAttribute('data-fntype');
      var targetElems;
      if(type === 'enemy' || type === 'line') targetElems = target.getElementsByClassName('if');
      else targetElems = target.getElementsByClassName('func');
      // find the li that's nearest to the cursor and insert placeholder bofore it
      var targets = Array.prototype.slice.call( targetElems, 0 ).filter(function(el){ return !ancestor(el, dragEl) });
      // calculate vertical distances
      var dists = targets.map(function(t){
        return [t, Math.abs(event.pageY - t.getBoundingClientRect().top), Math.abs(event.pageX - t.getBoundingClientRect().left)];
      });
      // find the nearest
      var nearest = dists.reduce(function(prev, curr){
        return (prev[1]+prev[2] < curr[1]+curr[2] ? prev : curr);
      });
  ...
  var startDrag = function(element, event){
    ...
    // only one if block is possible
    if(dragEl.parentNode){
      var type = dragEl.getAttribute('data-fntype');
      if(type === 'enemy' || type === 'line'){
        dragEl.parentNode.getElementsByTagName('li')[1].setAttribute('class', 'end if');
        console.log(dragEl.parentNode.getElementsByTagName('li'));
      }
    }
  ...
  var stopDrag = function(event){
    ...
    if(placeholder.parentNode){
      ...
      var type = dragEl.getAttribute('data-fntype');
      if(type === 'enemy' || type === 'line'){
        dragEl.parentNode.getElementsByTagName('li')[2].setAttribute('class', '');
      }
    }
